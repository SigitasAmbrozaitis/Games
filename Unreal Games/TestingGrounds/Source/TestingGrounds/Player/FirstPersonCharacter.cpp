...
void AFirstPersonCharacter::BeginPlay()
{
	// Call the base class  
	Super::BeginPlay();
	if (!ensure(GunBlueprint)) { UE_LOG(LogTemp, Error, TEXT("No Gun Blueprint")) return; }
	Gun = GetWorld()->SpawnActor<AGun>(GunBlueprint);
	Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
	Gun->AnimInstanceTP = Mesh1P->GetAnimInstance();
...
{
	UE_LOG(LogTemp, Warning, TEXT("Character Firing"))
	Gun->OnFire();
}
